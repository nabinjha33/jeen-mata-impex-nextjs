...
export default function MyOrders() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setCurrentUser(user);
      if (user?.email) {
        const orderData = await Order.filter({ dealer_email: user.email }, '-created_date');
        setOrders(orderData);
      }
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <Layout currentPageName="MyOrders">
        <div className="p-6">Loading your orders...</div>
      </Layout>
    );
  }
                                ...
                                <html>
                                  <head>
                                    ...
                                    <style>
                                      ...
                                      .header { border-bottom: 2px solid #dc2626; padding-bottom: 20px; margin-bottom: 20px; }
                                  ...
                                  <body>
                                    <div class="header">
                                      <h1>Order Slip - ${order.order_number}</h1>
                                      <p>Date: ${format(new Date(order.created_date), 'MMMM d, yyyy')}</p>
                                    </div>
                                    <table>
                                      <thead>
                                      ...
                                      <tbody>
                                        ${order.product_items.map((item: any) => `
                                          <tr>
                                            ...
                                            <td>${item.quantity}</td>
                                            <td>NPR ${item.unit_price_npr.toLocaleString()}</td>
                                            <td>NPR ${(item.unit_price_npr * item.quantity).toLocaleString()}</td>
                                          </tr>
                                        `).join('')}
                                      </tbody>
                                    </table>
                                    <div class="total">Total: NPR ${order.total_amount_npr.toLocaleString()}</div>
                                  </body>
                                </html>
                              `;
