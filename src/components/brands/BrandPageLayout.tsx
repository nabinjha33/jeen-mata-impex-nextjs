...
const brandThemes = {
  'FastDrill': {
    ...
    special: 'from-blue-500 to-blue-700'
  },
  'Spider': {
    ...
    special: 'from-red-500 to-orange-600'
  },
  'Gorkha': {
...
export default function BrandPageLayout({ brand }: BrandPageLayoutProps) {
  ...
  const theme = brandThemes[brand.name as keyof typeof brandThemes] || brandThemes['FastDrill'];
  ...
  const ProductCard = ({ product, index }: { product: any, index: number }) => (
    ...
    >
      <Link
        href={`${createPageUrl("ProductDetail")}?slug=${product.slug}&from=${brand.name}Brand`}
          ...
          <div className="relative h-56 overflow-hidden">
            ...
            {product.featured && (
              <div className="absolute top-4 right-4 z-20">
                <Badge className={`bg-gradient-to-r ${(theme as any).special || theme.primary} text-white border-0 shadow-lg`}>
      ...
      <div className="bg-white/80 backdrop-blur-sm border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-sm">
            <Link href={createPageUrl("Home")} className={`text-gray-600 hover:text-${theme.accent} transition-colors font-medium`}>
              Home
            </Link>
            <span className="text-gray-400">/</span>
            <Link href={createPageUrl("Brands")} className={`text-gray-600 hover:text-${theme.accent} transition-colors font-medium`}>
              Brands
            </Link>
            <span className="text-gray-400">/</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-8">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4">
                <Link href={`${createPageUrl("Products")}?brand=${encodeURIComponent(brand.name)}`}>
                  <Button size="lg" className="h-14 px-8 bg-white/90 hover:bg-white text-gray-900 hover:shadow-2xl transition-all duration-300 font-semibold">
                    <Package className="w-5 h-5 mr-2" />
                    Explore Products
                  </Button>
                </Link>
                <Link href={createPageUrl("DealerLogin")}>
                  <Button size="lg" variant="outline" className="h-14 px-8 border-2 border-white/50 text-white hover:bg-white/10 hover:border-white backdrop-blur-sm transition-all duration-300 font-semibold">
      ...
      <section className={`py-20 lg:py-28 ${theme.gradient} text-white relative overflow-hidden`}>
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <Link href={createPageUrl("DealerLogin")}>
              <Button size="lg" className="h-16 px-10 bg-white/90 hover:bg-white text-gray-900 hover:shadow-2xl transition-all duration-300 font-bold text-lg rounded-2xl">
                <Users className="w-6 h-6 mr-3" />
                Become a Dealer
              </Button>
            </Link>
            <Link href={createPageUrl("Products")}>
              <Button size="lg" variant="outline" className="h-16 px-10 border-2 border-white/50 text-white hover:bg-white/10 backdrop-blur-sm transition-all duration-300 font-bold text-lg rounded-2xl">
